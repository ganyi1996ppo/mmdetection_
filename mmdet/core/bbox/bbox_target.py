...
def soft_bbox_target_single(pos_bboxes,
                            neg_bboxes,
                            pos_gt_bboxes,
                            pos_gt_labels,
                            diluted_inds,
    ...
    labels = pos_bboxes.new_zeros((num_samples, cfg.classes), dtype=torch.long)
    label_weights = pos_bboxes.new_zeros(num_samples)
    bbox_targets = pos_bboxes.new_zeros(num_samples, 4)
    bbox_weights = pos_bboxes.new_zeros(num_samples, 4)
    if num_pos > 0:
        labels[torch.arange(num_pos), pos_gt_labels] = 1
        labels[diluted_inds, diluted_labels] = diluted_weights
