...
class MaskIoUHead_CA(nn.Module):
    """Mask IoU Head.
    ...
    def __init__(self,
        ...
        super(MaskIoUHead_CA, self).__init__()
        ...
        self.fc_mask_iou = nn.Linear(self.fc_out_channels, 1)
        self.relu = nn.ReLU()
    ...
    def forward(self, mask_feat, mask_pred):
        ...
        _,_,H,W = mask_pred.size()
        mask_pred_pooled = F.adaptive_max_pool2d(mask_pred)
        ...
        return mask_out, mask_iou

    @force_fp32(apply_to=('mask_iou_pred', ))
    def loss(self, mask_pred,  mask_iou_pred, mask_targets, mask_iou_targets):
        loss = dict()
        ...
        _,_,h,w = mask_pred.size()
        # mask_targets_resize = F.interpolate(mask_targets, (h,w))
        mask_targets_resize = mmcv.imresize(mask_targets,(w,h))
    ...
    def get_target(self, sampling_results, gt_masks, mask_pred, mask_targets, rcnn_train_cfg):
        ...
        _,_,H,W = mask_pred.size()
        mask_targets = F.interpolate(mask_targets[:,None,:,:], (H,W))
        ...
        area_ratios = torch.cat(list(area_ratios))[:,None]
    ...
    def get_mask_scores(self, mask_iou_pred, det_bboxes, det_labels):
        ...
        mask_scores = mask_iou_pred[inds] * det_bboxes[inds, -1]
