...
class FCNMaskHead_CA(nn.Module):
    ...
    def __init__(self,
        ...
        super(FCNMaskHead_CA, self).__init__()
    ...
    def get_all_target(self, sampling_results, gt_mask, rcnn_train_cfg):
        proposals = [torch.cat(res.pos_bboxes,res.neg_bboxes[:res.pos_bboxes.size(0)]) for res in sampling_results]

        assigned_gt_inds = [torch.cat([res.pos_assigned_gt_inds,torch.full(res.pos_bboxes.size(0), -1)])
