...
class TwoStageDetector_back(BaseDetector, RPNTestMixin, BBoxTestMixin,
    ...
    def forward_dummy(self, img):
        ...
        if self.with_bbox:
            ...
            mask_feats = F.adaptive_max_pool2d(mask_pred, (H,W))
            bbox_feats = torch.cat([bbox_feats, mask_feats], dim=1)
            if self.with_shared_head:
                bbox_feats = self.shared_head(bbox_feats)
            cls_score, bbox_pred = self.bbox_head(bbox_feats)
            outs = outs + (cls_score, bbox_pred)
    ...
    def forward_train(self,
        ...
        if self.with_mask:
            if not self.share_roi_extractor:
                ...
                mask_feats = self.mask_roi_extractor(
                    x[:self.mask_roi_extractor.num_inputs], rois) #pos_rois)
                if self.with_shared_head:
                    mask_feats = self.shared_head(mask_feats)
            # else:
            #     pos_inds = []
            ...
            neg_labels = [torch.full_like(res.neg_inds, -1) for res in sampling_results]
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        mask_pred = self.simple_test_mask(x, img_meta,
                                          proposal_list,
                                          rescale = rescale)
        ...
        if not self.with_mask:
            ...
            segm_results = self.mask_head.get_seg_masks(mask_pred,
                                                        det_labels,
                                                        self.test_cfg.rcnn,
